...
void R_SetViewSize (int blocks, int detail)
{
  ...
  else if (blocks<minblocks) blocks=minblocks;
...
void R_ExecuteSetViewSize (void)
{
  ...
  if (setblocks == maxblocks)
  ...
  else
    {
      ...
      viewheight = (setblocks*(screenheight-SBARHEIGHT)/(maxblocks-1));
      if (setblocks < maxblocks-1) {
	      viewheight &= ~15;
      }
    }
  ...
  /* commented out because of a FloatingPoint Exeption !!! (div by zero) */
  ...
  */
  ...
  skyiscale = FixedDiv(FRACUNIT, pspriteyscale);
    
  /*
    pspritescale = FixedDiv( FRACUNIT*viewwidth, screenwidth );
    pspriteiscale = FixedDiv( FRACUNIT*screenwidth, viewwidth );
  */
  ...
  for (i=0 ; i< LIGHTLEVELS ; i++)
      ...
      for (j=0 ; j<MAXLIGHTSCALE ; j++)
	{
	  ...
	  /*	  level = startmap - FixedDiv( j*screenwidth, (viewwidth<<detailshift)/DISTMAP ); */
	  if (level < 0)
	    level = 0;
	  if (level >= NUMCOLORMAPS)
	    level = NUMCOLORMAPS-1;
