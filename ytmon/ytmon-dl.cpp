...
bool download_video_bitchute(const Json &video) {
    ...
    {
        string cmd = string("youtube-dl --no-check-certificate --no-mtime --continue --add-metadata --write-all-thumbnails --embed-thumbnail --write-info-json --embed-subs --all-subs --limit-rate=2000K --output '%(id)s.mp4' ") + invoke_url;
        int status = system(cmd.c_str());
        if (WIFSIGNALED(status)) should_stop = true;
